    ...
    ui(new Ui::DisplayData)
{
    ...
    mAxisX->setLabelsAngle(90);                                        // x轴显示的文字倾斜角度
    mAxisX->setTickCount(61);                                          // 轴上点的个数
    QFont FontX("Times",6,2,false);                                   //设置X轴字体样式
    mAxisX->setLabelsFont(FontX);
    mAxisX->setRange(curDateTIme, curDateTIme.addSecs(60));            //X轴范围
    // y轴电压
...
void DisplayData::slotBtnClear()
{
    ...
    mSeriesT->clear();
    mAxisX->setRange(curDateTIme, curDateTIme.addSecs(60));     //X轴重置
    pointCount = 0;             //点数置零
}
...
void DisplayData::slotBtnStartAndStop()
{
    ...
    if(mtimer->isActive())
    {
        ...
        mAxisX->setMax(curDateTIme.addSecs(60));              // 更新X轴范围
        mtimer->stop();
...
void DisplayData::slotTimeout()
{
    ...
    if(pointCount > 60)                                         //图像坐标点超过20个更新坐标轴X
    {
        //mSeriesH->remove(0,0);mSeriesT->remove(0);
        mAxisX->setMin(curDateTIme.addSecs(-59));
