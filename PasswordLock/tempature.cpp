...
tempature::tempature(QWidget *parent) :
    ...
    ui(new Ui::tempature)
{
    /*下位机串口和ESP8266传过来的数据以及标志位*/
    Serial_T=0;                             //串口电压数据
    ESP8266_T=0;                            //ESP8266速度数据
    Serial_Flag=0;                          //串口电压标志
    ...
    m_minValue = 0;                         //最小刻度数值
    m_maxValue = 3300;                      //最大刻度数值
    m_scaleMinor = 5;                       //每个刻度分成5个分度
    m_value=Serial_T;                       //表盘显示的数据
}
...
void tempature ::drawScaleNum(QPainter *painter)
{
    ...
    for(int i=0; i<=10; i+=1)
    {
        ...
        QString value =QString( "%1" ).arg(i*330);
        //字体大小及位置要微调
...
void tempature::drawNumericValue(QPainter *painter)
{
    ...
    if(Serial_Flag)
        qDebug() << "电压" << Serial_T;
        m_value=Serial_T;                                   //接收串口数据
    //ESP8266传过来的数据
    if(ESP8266_Flag)
        m_value=ESP8266_T;                                  //接收ESP8266数据

    QString str=" ";
    str=str.number(m_value,'g',4);                          //后两个参数是格式和精度
