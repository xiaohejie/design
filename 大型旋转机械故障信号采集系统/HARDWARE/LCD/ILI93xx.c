...
//显示河海大学
void LCD_ShowHOHAI(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
	for(t=0;t<csize;t++)
	{   
		temp=HOHAI[num][t];	//调用河海大学
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
			else if(mode==0)LCD_Fast_DrawPoint(x,y,WHITE);
			temp<<=1;
			y++;
			if(y>=lcddev.height)return;		//超区域了
			if((y-y0)==size) 
...
//调用机械振动监测终端
void LCD_ShowHEAD(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 32,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
	for(t=0;t<csize;t++)
	{   
		temp=HEAD[num][t];	//调用河海大学
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
			else if(mode==0)LCD_Fast_DrawPoint(x,y,WHITE);
...
//显示ADC值
void LCD_ShowADC(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 32,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//显示电压值（可用于）
void LCD_ShowChinese(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//显示电压
void LCD_Showdianya(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//速度
void LCD_Showsudu(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//加速度
void LCD_Showjiasudu(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//频率
void LCD_ShowHz(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//key0：速度传感器
void LCD_Sudu(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//key1：加速度传感器
void LCD_Jiasudu(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//key2：波形图
void LCD_Boxingtu(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//key2：波形图
void LCD_Boxing(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//信号故障
void LCD_Signal(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//齿轮故障
void LCD_Chilun(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//时域分析
void LCD_shiyu(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//频域分析
void LCD_pinyu(u16 x,u16 y,u16 width,u16 height,u8 num)
{         
 u8 temp,t1,t,size = 24,mode=0;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size);		//得到字体一个字符对应点阵集所占的字节数	
...
//主界面
void LCD_showInterface()
{	 
	 u8 i;
	 for(i=0;i<8;i++)  //显示机械振动检测终端
	 {
		 LCD_ShowHEAD(100 + i*34,10,32,32,i);			 
	 }
	 for(i=0;i<6;i++)   //显示河海大学研制
	 {
		 LCD_ShowHOHAI(150 + i*26,52,24,24,i);			 
	 }
	 for(i=8;i<12;i++)   //显示可用于
	 {
		 LCD_ShowChinese(i*26-8*26,105,24,24,i);			 
	 }
	 for(i=4;i<10;i++){	//速度传感器135
			LCD_Sudu(i*26-4*26+26*2,195-60,24,24,i);
	 }
	 for(i=4;i<11;i++){	//加速度传感器165
			LCD_Jiasudu(i*26-4*26+26*10,195-60,24,24,i);
	 }
	 
	 for(i=12;i<16;i++)   //显示可测量
	 {
		 LCD_ShowChinese(i*26-12*26,200-20,24,24,i);			 
	 }
	 for(i=2;i<4;i++)   //显示当前电压
	 {
		 LCD_Showdianya(i*26-2*26+26*2,230-20,24,24,i);			 
	 }
	 for(i=0;i<2;i++)   //显示当前频率
		{
			LCD_ShowHz(i*26+26*10,230-20,24,24,i);	 		 
		}
	 for(i=0;i<2;i++)   //显示当前速度/加速度
	 {
		 LCD_Showsudu(i*26+26*2,260-20,24,24,i);		 	 
	 }
	 for(i=16;i<19;i++)   //显示电压差
	 {
		 LCD_ShowChinese(i*26+26*2+26*8-16*26,260-20,24,24,i);			 
	 }
	 for(i=0;i<3;i++)   //显示当前速度/加速度
	 {
		 LCD_Showjiasudu(i*26+26*2,290-20,24,24,i); 			 
	 }
	 for(i=19;i<23;i++)   //显示电压最值
	 {
		 LCD_ShowChinese(i*26+26*2+26*8-19*26,290-20,24,24,i);			 
	 }
	 LCD_ShowString(0,5+76,520,16,16,"-------------------------------------------------------------------------");
	 
 }
//速度传感器界面
void LCD_showsuduface(){
	u8 i;
	LCD_Clear(WHITE);	 		//先进行清屏操作
	for(i=9;i<14;i++)  //显示速度传感器
	 {
		 LCD_ShowHEAD(10 + i*34 - 7*34,10,32,32,i);			 
	 }
	for(i=0;i<6;i++)   //显示河海大学研制
	 {
		 LCD_ShowHOHAI(83 + i*26,52,24,24,i);			 
	 }
	 for(i=0;i<8;i++)   //显示当前ADC
	 {
		 LCD_ShowChinese(i*26,105,24,24,i);			 
	 }	
	 for(i=0;i<7;i++)   //显示当前电压
	 {
		 LCD_Showdianya(i*26,150,24,24,i);			 
	 }
	 for(i=0;i<10;i++)   //显示key2：波形图
	 {
		 LCD_Boxing(i*26,300,24,24,i);			 
	 }
...
//加速度传感器界面
void LCD_showjiasuduface(){
	u8 i;
	LCD_Clear(WHITE);	 		//先进行清屏操作
	for(i=8;i<14;i++)  //显示加速度传感器
	 {
		 LCD_ShowHEAD(25 + i*34 - 7*34,10,32,32,i);			 
	 }
	for(i=0;i<6;i++)   //显示河海大学研制
	 {
		 LCD_ShowHOHAI(83 + i*26,52,24,24,i);			 
