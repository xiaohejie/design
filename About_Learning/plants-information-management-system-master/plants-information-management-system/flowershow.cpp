...
int flowershow::getcost(QString flowername){
    ...
    QString selectSql = "SELECT SUM(cost) FROM plant_info WHERE TYPE = '" + flowername + "'" ;
    QSqlQuery query;
...
int flowershow::getnum(QString flowername){
    ...
    QString selectSql = "SELECT SUM(quantity) FROM plant_info WHERE TYPE = '" + flowername + "'" ;
    QSqlQuery query;
    query.exec(selectSql);
    if(query.next()){
...
std::tuple<QString,QString,QString> flowershow::getcompany(QString flowername){
    ...
    QString selectSql;
    //最大数量
    selectSql = "SELECT MAX(quantity)FROM plant_info WHERE TYPE = '" + flowername + "'" ;
    QSqlQuery query;
    query.exec(selectSql);
    if(query.next()){
    ...
    //公司名
    selectSql = "SELECT company  FROM plant_info WHERE TYPE = '" + flowername + "'"  +  "AND  quantity =  " + companynum ;
    ...
    //公司数量
    selectSql = "SELECT COUNT(DISTINCT company) FROM plant_info WHERE TYPE = '" + flowername + "'" ;
    QSqlQuery query3;
...
std::vector<std::pair<QString,int>>flowershow:: getgrowth(QString flowername){
    ...
    for(auto i : level){
        int num;
        QString selectSql =  "SELECT SUM(quantity) FROM plant_info  WHERE TYPE = '" + flowername + "'"  + " AND   growth = " + "'" + i + "';";
        qDebug()<<selectSql;
        QSqlQuery query;
        query.exec(selectSql);
...
*/
    ...
    for(auto i : level){
        int num;
        QString selectSql = "SELECT SUM(quantity) FROM plant_info  WHERE TYPE = '" + flowername + "'" + "AND watering =  " + "'" + i + "';";
        QSqlQuery query;
        query.exec(selectSql);
        if(query.next()){
            num = query.value(0).toInt();
...
*/
    ...
    for(auto i : level){
        int num;
        QString selectSql ="SELECT SUM(quantity) FROM plant_info  WHERE TYPE = '" + flowername + "'" + "AND fertilization =  " + "'" + i + "';";
        QSqlQuery query;
        query.exec(selectSql);
        if(query.next()){
            num = query.value(0).toInt();
