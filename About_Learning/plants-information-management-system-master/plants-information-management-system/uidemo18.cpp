    ...
    ui(new Ui::UIDemo18)
{
    ...
    //槽函数：功能实现
    connect(ui->btn_Product, SIGNAL(clicked()), this, SLOT(btn_Product_Clicked_slots()));   //信息采集
    connect(ui->btn_Info, SIGNAL(clicked()), this, SLOT(btn_Info_Clicked_slots()));         //档案记录
    connect(ui->btn_Research, SIGNAL(clicked()), this, SLOT(btn_Research_Clicked_slots())); //信息查询
    connect(ui->btn_Map, SIGNAL(clicked()), this, SLOT(btn_Map_Clicked_slots()));           //时频分析
    connect(ui->btn_Analysis, SIGNAL(clicked()), this, SLOT(btn_Analysis_Clicked_slots())); //故障诊断
    connect(ui->btn_Settings, SIGNAL(clicked()), this, SLOT(btn_Settings_Clicked_slots())); //系统设置
    connect(ui->btn_QGIS, SIGNAL(clicked()), this, SLOT(btn_QGIS_Clicked_slots()));         //QGIS
}
...
{
    ui->labTitle->setText("齿轮故障诊断信息管理系统");
    ...
    //主窗体背景
    qss.append(QString("QWidget#widgetMain{border-image:url(%1);}").arg(":/image/_homePage.jpg"));
...
void UIDemo18::initNav()
{
    ...
    listColorBg << "#1570A5" << "#16A085" << "#C0392B" << "#047058" << "#9B59BB" << "#34495E" << "#F5DEB3";
    QList<QString> listColorText;
    listColorText << "#FEFEFE" << "#FEFEFE" << "#FEFEFE" << "#FEFEFE" << "#FEFEFE" << "#FEFEFE" << "#FEFEFE";

    QList<QChar> listChar;
    listChar << 0xf030 << 0xf108 << 0xf002 << 0xf03e << 0xf0e8 << 0xf085 << 0xf041;

    QList<QString> listText;
    listText << "信息采集"<< "档案记录" << "信息查询" << "时频分析" << "故障诊断" << "系统设置" << "QGIS";

    //槽函数，功能实现
    btns << ui->btn_Product << ui->btn_Info << ui->btn_Research << ui->btn_Map << ui->btn_Analysis << ui->btn_Settings << ui->btn_QGIS;
...
//功能：QGIS
void UIDemo18::btn_QGIS_Clicked_slots() {
    QProcess process(this);
    QString str ="F:/present/OSGeo4W/bin/qgis-ltr-bin.exe";    //加可执行文件路径
...
void UIDemo18::on_btn_AddFlower_clicked()
{
    ...
    if(ui->comboBox->currentIndex() == 0){
        QMessageBox::information(this,"Error","<font size='12' color='red'>请选择齿轮</font>",QMessageBox::Ok);
    }
...
void UIDemo18::on_pushButton_map_clicked()
{
    ...
    QMessageBox::information(this,"success","<font size='12' color='red'>地图url已更改</font>",QMessageBox::Ok);
