    ...
    if(ui->lineEdit_ID->text() == ""){
        ...
        if(ui->lineEdit_PWord->text() == ui->lineEdit_ReplyPWord->text()){
            ...
            if(query.next()){
                ...
                query.prepare("insert into plants_management.operator_info(UserId,Password,Master)"
                              "values(:UID,:PWord,:master)");
                query.bindValue(":UID", ui->lineEdit_ID->text());
                query.bindValue(":PWord",ui->lineEdit_PWord->text());
                query.bindValue(":master", 1 - ui->comboBox->currentIndex());
                query.exec();
