    ...
    , ui(new Ui::selectWidget)
{
    ...
    //operator_info
    //plant_info_copy
    //model->setTable("plant_info");

    m_pTabModel->setTable("plant_info");
...
void selectWidget::on_selectbth_clicked()
{
    ...
    // QString basesql = "SELECT * FROM plant_info WHERE 1=1 ";
    QString basesql =  "SELECT * FROM plant_info  ";
    QString finallsql;
    if(res.length() !=0)
    ...
    // select a,b,c   from plant_info order by d;
...
void selectWidget::on_shouge_clicked()
{
    ...
    QList<std::pair<QString,QString>> res =  getalltext();
    QString basesql =  "UPDATE plant_info SET ";
    QString finallsql;
    if(res.length() !=0){
        for(int i = 0;i<res.length();i++){
...
void selectWidget::on_viewtab_clicked()
{
    ...
    QString basesql =  "SELECT * FROM plant_info ORDER By id ASC ";
...
void selectWidget::on_del_clicked()
{
    ...
    if(Global::type =="1"){
        ...
        if( ui->id->currentText() !=""){
            ...
            QList<std::pair<QString,QString>> res =  getalltext();
            QString basesql =  "DELETE FROM  plant_info WHERE  id  = " +  id ;
