...
//                   缺水
//                   急缺水
//                  "SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + flowername + "' + "AND `watering` ='浇水情况正常';"
std::map<QString,std::vector<int>> sqldataread::getwater(){
    ...
    for(int i = 0;i<tablename.size();i++){
        QStringList selectSql = {QString("SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + ChinaTablename[i] + "' AND `watering` ='浇水情况正常';"),QString("SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + ChinaTablename[i] + "' AND `watering` ='缺少量水';"),QString("SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + ChinaTablename[i] + "' AND `watering` ='急缺水';")};
        QSqlQuery query;
        std::vector<int> tmp1;
...
std::map<QString,std::vector<int>> sqldataread::getfeiliao(){
    ...
    for(int i = 0;i<tablename.size();i++){
        QStringList selectSql = {QString("SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + ChinaTablename[i] + "' AND `fertilization` ='肥料情况正常';"),QString("SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + ChinaTablename[i] + "' AND `fertilization` ='缺少少量肥料';"),QString("SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + ChinaTablename[i] + "' AND  `fertilization` ='急缺肥料';")};
        QSqlQuery query;
        std::vector<int> tmp1;
        for(auto sql:selectSql){
...
std::map<QString,std::vector<int>> sqldataread::getgrowth(){
    ...
    for(int i = 0;i<tablename.size();i++){
        QStringList selectSql = {QString("SELECT SUM(`quantity`)  FROM plant_info  WHERE  TYPE = '" + ChinaTablename[i] + "'   AND   `growth` ='A';"),
                                 QString("SELECT SUM(`quantity`)  FROM plant_info  WHERE  TYPE = '" + ChinaTablename[i] + "'   AND   `growth` ='B+';"),
                                 QString("SELECT SUM(`quantity`)  FROM plant_info  WHERE  TYPE = '" + ChinaTablename[i] + "'   AND   `growth` ='B';"),
                                 QString("SELECT SUM(`quantity`)  FROM plant_info  WHERE  TYPE = '" + ChinaTablename[i] + "'   AND   `growth` ='C+';"),
                                 QString("SELECT SUM(`quantity`)  FROM plant_info  WHERE  TYPE = '" + ChinaTablename[i] + "'   AND   `growth` ='C';"),
                                 QString("SELECT SUM(`quantity`)  FROM plant_info  WHERE  TYPE = '" + ChinaTablename[i] + "'   AND   `growth` ='D';")};
        QSqlQuery query;
        std::vector<int> tmp1;
        for(auto sql:selectSql){
...
std::pair<QString,std::vector<int>> sqldataread:: getFlowerGrowth(QString flowerName){
    ...
    db.setDatabaseName(Global::dataBaseName);
    db.open();
    QStringList selectSql = {QString( "SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + flowerName + "' AND  `growth` ='A';"),
                             QString( "SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + flowerName + "' AND  `growth` ='B+';"),
                             QString( "SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + flowerName + "' AND  `growth` ='B';"),
                             QString( "SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + flowerName + "' AND  `growth` ='C+';"),
                             QString( "SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + flowerName + "' AND  `growth` ='C';"),
                             QString( "SELECT SUM(`quantity`) FROM plant_info  WHERE  TYPE = '" + flowerName + "' AND  `growth` ='D';")};
    QSqlQuery query;
    std::vector<int> tmp1;
    for(auto sql:selectSql){
        query.exec(sql);
